...
<section class="review-section animate-fade-up" class:is-header={isHeader} class:form-open={showForm}>
	<div class="review-content">
		<div class="text-side">
			{#if !isHeader}
				<span class="tag tag--yellow">Hardware</span>
			{/if}
			<h2>CAD Review</h2>
			<p>
			...
			<p class="reminder-box">
				<strong>Reminder:</strong> Give viewer access to 
				<code class="email-code">ftcblueprint@gmail.com</code>. We will share your email with feedback.
			</p>
			
			{#if !showForm}
				<button class="btn-primary" style={'color: var(--bg)'} onclick={() => showForm = true}>
					Get Review
			...
			<div class="form-side" transition:fade>
				...
				{:else}
					<form onsubmit={handleSubmit} class="review-form">
						...
						<input type="text" name="cad_link" placeholder="Link to CAD (Onshape, Step, etc.)" required />
						<textarea name="notes" placeholder="Notes (Specific subsystems, weight issues...)" rows="2"></textarea>
...
<style>
	...
	.review-section.form-open.is-header {
		max-width: 600px;
	}
	...
	.reminder-box {
		...
		background: rgba(189, 157, 68, 0.08); /* Yellow background tint */
